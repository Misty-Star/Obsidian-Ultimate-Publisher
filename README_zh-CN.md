...
Ultimate Publisher 保持 provider 身份扁平且稳定：每个目标只持久化一个 `target.provider`，例如 `wordpress`、`github-hugo` 或 `gitlab-vitepress`。Marketplace 分类（`common`、`wordpress`、`github`、`gitlab`、`web` 等）只是从 provider definition 派生出来的 UI 分组元数据，不是运行时持久化层级。
