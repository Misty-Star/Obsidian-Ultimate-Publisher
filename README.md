...
Generic legacy `github` and `gitlab` static-site target ids are no longer accepted; create a generator-specific target such as `github-hugo` or `gitlab-vitepress` instead. Local File/Local Filesystem publishing has been removed from the product surface. Media upload and delete are intentionally not exposed for static-site repository targets yet.
